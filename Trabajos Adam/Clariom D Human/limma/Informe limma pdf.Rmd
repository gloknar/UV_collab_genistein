...
En oligo, el comando rma nos devuelve un objeto de tipo ExpressionSet (similar al objeto datos_crudos_microarrays) con los datos preprocesados en el bolsillo \verb|eSet_normalizado@assayData$exprs|. Procedemos al RMA:
...
En resumen, sólo el PCA de 2 dimensiones es informativo de la posible distribución de las muestras. El t-SNE y el UMAP en 3D no son concluyentes (los datos parecen mezclarse).
...
Antes de nada tenemos que indicar a limma mediante el hueco phenoData a qué grupo (en este caso, factor condicion) pertenece cada muestra. Podemos ver que ya hemos hecho eso en pasos anteriores:
...
Ahora tenemos que crear una __matriz de diseño__, o sea una matriz de valores del factor que agrupa los datos. Los t-tests, ANOVAs y GLMs necesitan dicha matriz para saber qué muestras pertenecen a qué grupo. Puesto que limma usa un t-test o un ANOVA, necesita dicha matriz de diseño. puedes crearla con el método \verb|model.matrix()|. Esta función toma como argumento un modelo de fórmula (_model formula_). La virgulilla (~) especifica el lado derecho de la fórmula. Si usas como fórmula ~0, limma calculará sólo la expresión génica media de cada grupo.
...
Podemos ver la expresión génica media de cada grupo en formato log<sub>2</sub> en el bolsillo \verb|$coefficients| del modelo lineal:
