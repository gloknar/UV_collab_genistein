...
En este documento se muestra cómo analizar los archivos con extensión .CEL resultantes de secuenciar muestras biológicas en los microchips _Clariom D Human_ de Affymetrix. Llevaremos a cabo este análisis en R 3.6 con el paquete de Bioconductor \verb|oligo| versión 1.50.0. La versión de Biocondcutor empleada es la 3.10.
...
Al cargar los archivos \verb|.CEL| con \verb|read.celfiles()|, se almacena en el bolsillo \verb|datos_crudos_microarrays@assayData$exprs| la matriz de intensidades crudas de los fluoróforos Cy3 y Cy5 que hibridan con las distintas sondas de cada microarray. Esta matriz tiene tantas filas como sondas tiene el microarray, y tantas columnas como muestras hayamos secuenciado. Las funciones \verb|exprs()| e \verb|intensity()| del paquete \verb|oligo| simplifican el acceso a la misma (ambas funciones devuelven exactamente el mismo _output_).
...
Con el comando \verb|probeNames()| obtenemos los _transcription clusters_ a los que pertenecen las sondas de nuestro microchip, de manera que la primera sonda del microarray pertenece al _transcription cluster_ TC1300007722.hg.1: 
...
En R, el paquete oligo parece no poder sacar dichas métricas en microarrays de tipo _Clariom_, puesto que al ser nuevos, no llevan asociados archivos \verb|.CDF|. Quizás \verb|limma| o \verb|DESeq2| puedan tratar dichos microchips.
...
Para preprocesar y normalizar datos de microarrays, se suelen emplear el algoritmo RMA o derivados (_i.e._ GCRMA o SST-RMA). En nuestro caso vamos a usar el algoritmo RMA, implementado en \verb|oligo|. Este algoritmo consta de 3 pasos:

* __Background subtraction__. De acuerdo a la \href{http://bioconductor.org/packages/release/bioc/vignettes/oligo/inst/doc/oug.pdf}{documentación} de \verb|oligo|, el método de _background substraction_ implementado en RMA trata las sondas PM (_Perfect Match_, más informacion en [Flight _et al._, 2012](http://rmflight.github.io/affyMM/)) como una convolución de ruido y señal verdadera.
...
Para visualizar si hay genes diferencialmente expresados, podemos emplear el comando \verb|MAplot()| del paquete oligo. Los gráficos MA muestran en el eje X la intensidad media de una sonda dada a lo largo de los microarrays estudiados, y en el eje Y se observa el log\textsubscript{2}(fold change) o logaritmo en base dos del ratio de sobre/infraexpresión del gen al que mapea dicha sonda. 
...
Procedemos a detectar los genes diferencialmente expresados mediante las utilidades del paquete limma, ya que oligo carece de las funciones necesarias para ello. limma usa como _input_ el objeto que devuelve el comando \verb+rma()+. 
...
Ahora debes indicarle al paquete qué grupos quieres comparar. Para ello debes definir una __matriz de contrastes__ en la cual se especifiquen las comparaciones a realizar mediante el comando \verb|makeContrasts()|. Usando los nombres de las columnas de la matriz de diseño, puedes especificar el grupo control respecto al cual quieres comparar el grupo de interés (tratado con genisteína).
