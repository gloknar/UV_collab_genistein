...
volcanoplot(data.fit.eb, highlight = 0); abline(h = 1.301, v = c(-1,1), lty = 2)
```
...
Las células troncales tratadas con microvesículas provienen del mismo individuo (no son sets independientes), por lo que estamos tratando con datos pareados. Que los datos sean pareados significa que existe una relación entre las muestras de los distintos grupos estudiados. Es importante especificarle a `limma` si los datos son pareados o no, ya que en función de ello usará unos tests estadísticos u otros, a saber:

* Para dos grupos de datos: t-test pareado en lugar de un t-test normal
    
* Para más de 2 grupos de datos: ANOVA con medidas repetidas en lugar de un anova de una vía
