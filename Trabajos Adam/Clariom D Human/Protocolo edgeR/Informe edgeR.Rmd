...
Primero debemos instalar `edgeR`. Téngase en cuenta que en R 3.6 se instala la versión 1.50.0 de `oligo`, el cual depende de los paquetes `ff` versión 2.2.0 y `RSQLite` versión 2.1.4. Instalar versiones posteriores ocasionará fallos en la carga de `oligo` y en los análisis posteriores:

```{r instalacion edgeR y dependencias, eval = F}
# Instalación edgeR en R 3.6:
Sys.setenv(MAKE = "make -j4")
BiocManager::install("edgeR")
install.packages("httpuv", Ncpus = 4, type = "source")
